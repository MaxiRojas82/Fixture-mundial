...
from src.ui.notifications import EVENTS, is_enabled, set_enabled, is_favorite, toggle_favorite
...
def open_notif_dialog(page: ft.Page, service=None) -> None:
    # ── Sección eventos ──────────────────────────────────────────────────────
            ...
            ft.Container(
                content=ft.Row([
                    ...
                    ft.Switch(
                        ...
                        active_color=accent,
                        on_change=lambda e, et=etype: set_enabled(et, e.control.value),
                    ),
    ...
    # ── Sección equipos favoritos ────────────────────────────────────────────
    teams: list[str] = []
    if service:
        seen: set[str] = set()
        for m in service.matches:
            for t in (m.home, m.away):
                if t.id != 0 and t.name not in seen:
                    seen.add(t.name)
                    teams.append(t.name)
    ...
    def _hint_text() -> str:
        ...
        if n == 0:
            return "Sin favoritos — recibirás notificaciones de todos los equipos"
        return f"{n} equipo{'s' if n != 1 else ''} favorito{'s' if n != 1 else ''} seleccionado{'s' if n != 1 else ''}"
    ...
    def _toggle_chip(name: str) -> None:
        toggle_favorite(name)
        c = chip_refs.get(name)
        if c:
    ...
    for name in teams:
        ...
        chip = ft.Container(
            ...
            on_click=lambda _, n=name: _toggle_chip(n),
            ink=True,
        )
        chip_refs[name] = chip
        chips.append(chip)

    fav_section: list[ft.Control] = [
        ft.Container(
            content=ft.Row([
                ft.Icon(ft.Icons.STAR_ROUNDED, size=14, color=COLORS["accent"]),
                ft.Text("Equipos favoritos", size=13,
    ...
    dlg = ft.AlertDialog(
        ...
        content=ft.Container(
            content=ft.Column(
                event_rows + [ft.Divider(color=COLORS["card_border"], height=1)] + fav_section,
                spacing=4,
...
def _build_drawer(page: ft.Page, service=None) -> ft.NavigationDrawer:
    ...
    def _on_theme_toggle(_):
        ...
        # Reconstruir el tema para que on_secondary_container y otros colores
        # del ColorScheme reflejen el nuevo modo (afecta ícono/label de NavigationBar)
        page.theme = app_theme()
        # Flet skips on_route_change when route hasn't changed (__last_route dedup).
        # Reset it so route_change fires and rebuilds all widgets with new COLORS.
    ...
    def _on_notif_tap(_):
        if _drawer is not None:
            _drawer.open = False
            page.update()
        open_notif_dialog(page, service)
